...
import GameWrapper from "./GameSystem" // Assuming GameSystem is a component that provides gameProps
...
export default function DailyRoutineGame() {
  const [activities, setActivities] = useState([
    ...
    { id: 6, name: "Go to School", image: schoolImg, color: "bg-red-200", order: 5 }, // Fixed position for "Go to School"
  ])
  ...
  useEffect(() => {
    // Filter out 'Go to School' as it's not draggable and shuffle the rest
    const shuffled = [...activities.filter((a) => a.id !== 6)].sort(() => Math.random() - 0.5)
    setShuffledActivities(shuffled)
  }, [])

  useEffect(() => {
    // Game is completed when all draggable activities have been placed
  ...
  const handleDrop = (index, gameProps) => {
    if (draggingId !== null) {
      ...
      // Only allow drops for draggable activities (id 1-5) into routine slots (0-4)
      if (draggedActivity && draggedActivity.id !== 6 && index >= 0 && index <= 4) {
        setActivities((prevActivities) => {
          const nextActivities = prevActivities.map((activity) => {
            // If another activity was already in this slot, set its order to null (send it back to the source pool)
            if (activity.order === index && activity.id !== draggingId) {
              return { ...activity, order: null }
            }
            // If this is the dragged activity, assign its new order
            if (activity.id === draggingId) {
        ...
        gameProps.addPoints(15) // Award points for any valid placement
      }
      // Reset draggingId regardless of whether the drop was valid or not
      // (e.g., dropping a draggable item into the fixed "Go to School" slot will just do nothing)
      setDraggingId(null)
    }
  }

  const resetGame = () => {
    // Reset order for all activities except 'Go to School'
    const resetActivities = activities.map((activity) => (activity.id === 6 ? activity : { ...activity, order: null }))
    setActivities(resetActivities)

    // Reshuffle the draggable activities
      ...
      <div className="w-full max-w-6xl mt-20 mx-auto p-6 bg-gradient-to-b from-blue-50/20 to-purple-50/20 backdrop-blur-xl rounded-xl shadow-lg border border-white/20">
        ...
        {gameCompleted ? (
          <div className="mt-10 text-center animate-fade-in">
            <div className="text-4xl font-extrabold text-green-400 mb-4">🎉 Awesome Work! 🎉</div>
            <div className="p-6 bg-gradient-to-r from-yellow-100/20 to-yellow-200/20 rounded-xl shadow-lg mb-6 backdrop-blur-sm border border-white/20">
              <p className="text-2xl text-white font-semibold">You nailed your morning routine like a pro! 🌞👏</p>
              <p className="mt-2 text-lg text-white/80">Ready to play again or continue your journey?</p>
            </div>
            <button
              onClick={resetGame}
          ...
          <div className="flex flex-col space-y-2">
            ...
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-xl shadow-md border border-white/20">
              <div className="grid grid-cols-5 gap-3 -mb-4">
                {shuffledActivities.map(
                  (activity) =>
                    activity.order === null && ( // Only show activities not yet placed
                      <div
                        key={activity.id}
                      ...
                      >
                        <img src={activity.image || "/placeholder.svg"} alt={activity.name} className="w-48 h-40" />
                      </div>
                    ),
                )}
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`h-24 rounded-lg flex flex-col -mb-3 items-center justify-center ${
                        placedActivity ? placedActivity.color : "bg-gray-100/20 border-2 border-dashed border-gray-300"
                      }`}
                    ...
                    >
                      {placedActivity ? (
                        <>
                          <img
                            src={placedActivity.image || "/placeholder.svg"}
                            alt={placedActivity.name}
                            className="w-28 h-28"
                          />
                        </>
                      ) : (
